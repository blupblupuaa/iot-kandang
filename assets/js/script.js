...
function startSensorPolling(intervalMs) {
    ...
    async function poll() {
        ...
        try {
            ...
            if (typeof appendMonitorHistory === 'function' && json.saved) appendMonitorHistory(json.data);
...
// Profil: tidak ada JS carousel — layout sudah single-page grid
...
document.addEventListener("DOMContentLoaded", () => {
    const observerOptions = {
        root: null,
        rootMargin: '0px',
        threshold: 0.25 // Trigger ketika 25% elemen sudah masuk layar
    };
    ...
    const scrollObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Hapus class is-visible agar elemen fade out lagi
                entry.target.classList.remove('is-visible');
            }
        });
    }, observerOptions);

    const fadeElements = document.querySelectorAll('.profil-section');
